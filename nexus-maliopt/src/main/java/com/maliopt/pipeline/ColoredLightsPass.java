...
public class ColoredLightsPass {
    ...
    private static int populateLightsFromScene(World world) {
        ...
        for (BlockPos pos : BlockPos.iterate(
            ...
            float[] color = getLightColor(state);
            if (color != null) {
                float px = pos.getX() + 0.5f;
                float py = pos.getY() + 0.5f;
                float pz = pos.getZ() + 0.5f;
                lightBuffer.put(new float[]{px, py, pz, color[0], color[1], color[2], 1.5f, 12f});
                count++;
            }
        }
    ...
    private static float[] getLightColor(BlockState state) {
        if (state == null) return null;
        if (state.isOf(Blocks.TORCH))        return new float[]{1f, 0.5f, 0.1f};
        if (state.isOf(Blocks.SOUL_TORCH))   return new float[]{0.1f, 0.5f, 1f};
        if (state.isOf(Blocks.LAVA))          return new float[]{1f, 0.2f, 0.05f};
        if (state.isOf(Blocks.GLOWSTONE))     return new float[]{0.3f, 1f, 0.3f};
        if (state.isOf(Blocks.CAMPFIRE))      return new float[]{1f, 0.6f, 0.2f};
        if (state.isOf(Blocks.SOUL_CAMPFIRE)) return new float[]{0.2f, 0.6f, 1f};
        if (state.isOf(Blocks.LANTERN))       return new float[]{1f, 0.7f, 0.3f};
        return null;
    }
